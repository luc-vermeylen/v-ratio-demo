...
This issue is most consequential — and most likely to be overlooked — specifically when comparing a **Null model** against a model with a **condition-dependent parameter**. In this comparison, only one of the two models actually "needs" the condition split to estimate its parameters; it's tempting to therefore leave `CONDITIONS` empty for the Null model, since it doesn't use it for `VARYING_PARAMS` (which will result in an error and stop the script). Note that the pipeline cannot catch this within a single fit, since an empty `CONDITIONS` is a perfectly valid configuration on its own — the mismatch only becomes visible later, at `3_fit_compare.R`, via the `n_bins`warning described above. This is exactly the purpose of the decoupling: even though the Null model's parameters don't depend on `CONDITIONS`, it must still be evaluated using the same `CONDITIONS` as the Complex model, or the two G2/BIC values will be computed on different scales (see above). On the other hand, when comparing two models that both already use `VARYING_PARAMS` by condition (e.g. a model where `v` varies by Difficulty vs. one where `vratio` varies by Difficulty), this problem is less likely to arise naturally, since both models already require the data to be split in the same manner for the parameter to be fit (except when there is a third model with an even more complex design structure, e.g., where there is second factor in the design). 

### Best Practices for Model Comparison
> **To ensure valid model comparisons, the `CONDITIONS` vector must be identical across all candidate models.** You should always set `CONDITIONS` to reflect the most full design used by the most complex model in your comparison set, even for models that do not utilize those factors in `VARYING_PARAMS`. Perhaps you don't know your full design yet at the start of your fitting, but once you have identified the most complex model you wish to include in model comparisons, you can go back to more simple models and make sure you partition them in the same manner as your most complex model.
